...
game_t megamorpion(game_t game_data, uint8_t owninput, uint8_t oppsinput)
{
    ...
    */
    ...
    static uint8_t megacolor = LED_NOIR;        // couleur de la megagrille <=> state qui est le gagnant de la partie
    ...
    static uint8_t minicolor[3][3];             // couleur de chaque minigrille[x][y]
    ...
    switch (owninput) {
        ...
        case INPUT_A :
            if (isnan(xomatrix[megax][megay][minix][miniy])) { // si la case est libre
                ...
                colmatrix[megax*3+minix][megay*3+miniy] = JOUEUR1;
            } 
            ...
            */
            for (uint8_t i=0; i<3; i++) {
                for (uint8_t j=0; j<3; j++) {
                    if (minicolor[j][i] == LED_NOIR) {     // si ya deja une win il faut pas revérifier
                        minicolor[j][i] = check_win(xomatrix[i][j]);  // ? pourquoi minicolor[j][i] et pas [i][j]?? je sais pas mais ça marche alors nsm
                        
                        if (minicolor[j][i] == JOUEUR1) megamatrix[j][i] = 0;
                        if (minicolor[j][i] == JOUEUR2) megamatrix[j][i] = 1;
                    }

                    megacolor = check_win(megamatrix);
                }
            }

            if (megacolor == JOUEUR1) game_data.winlose = WIN;
            if (megacolor == JOUEUR2) game_data.winlose = LOSE;
            break;
...
static uint8_t check_win(float** mat) 
{
    ...
    for (i=0; i<3; i++) {
        ...
        if (!isnan(lin) && (lin == 0)) return JOUEUR1;
        else if (!isnan(lin) && (lin == 3)) return JOUEUR2;
        else if (!isnan(col) && (col == 0)) return JOUEUR1;
        else if (!isnan(col) && (col == 3)) return JOUEUR2;
        else if (!isnan(diag1) && (diag1 == 0)) return JOUEUR1;
        else if (!isnan(diag1) && (diag1 == 3)) return JOUEUR2;
        else if (!isnan(diag2) && (diag2 == 0)) return JOUEUR1;
        else if (!isnan(diag2) && (diag2 == 3)) return JOUEUR2;
    }

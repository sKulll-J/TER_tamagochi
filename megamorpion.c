            ...
            for (uint8_t i=0; i<3; i++) {
                for (uint8_t j=0; j<3; j++) {
                    if (miniwin[j][i] == LED_NOIR) {     // si ya deja une win il faut pas revérifier
                        ...
                        if (miniwin[j][i] == JOUEUR1) megamatrix[j][i] = 0;
                        if (miniwin[j][i] == JOUEUR2) megamatrix[j][i] = 1;
                    }

                    if (check_win(megamatrix) == JOUEUR1) game_data.winlose = WIN;
                    if (check_win(megamatrix) == JOUEUR2) game_data.winlose = LOSE;
                }
            }
